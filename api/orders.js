...
function processOrders(orders) {
  ...
  const products=[...pm.entries()].map(([name,v])=>{
    ...
    return {name,sku:v.sku,units:v.units,revenue:v.revenue,orders:v.orders.size,
      ...
      inv_c34:v.sku?(C34I[v.sku]||0):0,inv_c35:v.sku?(C35I[v.sku]||0):0,
      inv_c36:v.sku?(C36I[v.sku]||0):0,inv_c37:v.sku?(C37I[v.sku]||0):0,
      inv_c38:v.sku?(C38I[v.sku]||0):0,
      q_cont:qc,disponible:qc>0?qc-v.units:null};
  });

  // SKUs en planillas sin ventas — agregar para trazabilidad
  const soldSkus = new Set(products.map(p=>p.sku).filter(Boolean));
  const allInvSkus = new Set([
    ...Object.keys(C34I), ...Object.keys(C35I), ...Object.keys(C36I),
    ...Object.keys(C37I), ...Object.keys(C38I)
  ]);
  for (const sku of allInvSkus) {
    ...
    products.push({
      ...
      inv_c34:C34I[sku]||0, inv_c35:C35I[sku]||0, inv_c36:C36I[sku]||0,
      inv_c37:C37I[sku]||0, inv_c38:C38I[sku]||0,
