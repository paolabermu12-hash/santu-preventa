...
async function getAllOrders() {
  let orders = [];
  let url = '/admin/api/2024-01/orders.json?status=any&limit=250&created_at_min=2026-01-01T00:00:00-03:00&fields=name,created_at,cancelled_at,financial_status,fulfillment_status,tags,total_price,billing_address,line_items';
  while (url) {
...
// ARRIVED: contenedores que YA llegaron a Chile y tienen stock habilitado en Shopify.
// Una vez que un contenedor llega, sus productos salen del seguimiento de PREVENTA.
// >>> Para marcar un contenedor como llegado, agregar su numero aca. <<<
const ARRIVED = new Set(['34']);
// PREVENTA: contenedores activos que todavia NO llegan (los unicos relevantes para la vista Productos)
const PREVENTA = [...ACTIVE].filter(c => !ARRIVED.has(c));
const ARRIVED_LABELS = new Set([...ARRIVED].map(c => 'Contenedor ' + c));
...
function processOrders(orders) {
  // inv(): inventario de un SKU SOLO en contenedores que aun no llegan (preventa).
  // Los contenedores ya llegados (ARRIVED) no se cuentan: su stock ya es inventario normal.
  const INV_C = {'34':C34I,'35':C35I,'36':C36I,'37':C37I,'38':C38I};
  const inv = sku => PREVENTA.reduce((s,c) => s + (INV_C[c][sku]||0), 0);
  ...
  for (const o of orders) {
    ...
    // EXCLUSION TEMPRANA: pedidos anulados/reembolsados nunca se preparan
    if (o.cancelled_at) continue;
    if (['voided','refunded','expired'].includes(fin)) continue;
  ...
  // Adjuntar los contenedores en los que aparece el pedido entero (para detectar mix)
  const ocMap = new Map();
  for (const l of deduped) {
    if (!ocMap.has(l.order)) ocMap.set(l.order, new Set());
    ocMap.get(l.order).add(l.container);
  }
  for (const l of deduped) {
    l.order_containers = [...ocMap.get(l.order)].sort();
  }

  const pm=new Map();
  for (const l of deduped) {
    // Excluir lineas de contenedores ya llegados — su stock ya no es preventa
    if (ARRIVED_LABELS.has(l.container)) continue;
    const k=l.product_clean;
    if (!pm.has(k)) pm.set(k,{units:0,units_prep:0,revenue:0,orders:new Set(),cb:{},prices:[],sku:''});
    const v=pm.get(k);
    if(l.sku&&!v.sku)v.sku=l.sku;
    if(l.unit_price>0)v.prices.push(l.unit_price);
    // Lineas ya preparadas (resueltas con stock fisico): NO consumen del contenedor que viene.
    // Se registran aparte como referencia, pero no cuentan en units/revenue/cb.
    if (l.fulfillment_status === 'fulfilled') {
      v.units_prep += l.quantity;
      continue;
    }
    v.units+=l.quantity;v.revenue+=l.subtotal;v.orders.add(l.order);
    v.cb[l.container]=(v.cb[l.container]||0)+l.quantity;
  }
  // inv por contenedor SOLO si el contenedor sigue en preventa (los llegados van en 0)
  const invC = (sku,c) => (sku && PREVENTA.includes(c)) ? (INV_C[c][sku]||0) : 0;
  const products=[...pm.entries()].map(([name,v])=>{
    const qc=v.sku?inv(v.sku):0;
    return {name,sku:v.sku,units:v.units,units_prep:v.units_prep,revenue:v.revenue,orders:v.orders.size,
      ...
      inv_c34:invC(v.sku,'34'),inv_c35:invC(v.sku,'35'),
      inv_c36:invC(v.sku,'36'),inv_c37:invC(v.sku,'37'),
      inv_c38:invC(v.sku,'38'),
      q_cont:qc,disponible:qc>0?qc-v.units:null};
  });

  // SKUs en planillas de contenedores EN PREVENTA sin ventas — agregar para trazabilidad
  const soldSkus = new Set(products.map(p=>p.sku).filter(Boolean));
  const allInvSkus = new Set();
  for (const c of PREVENTA) for (const sku of Object.keys(INV_C[c])) allInvSkus.add(sku);
  for (const sku of allInvSkus) {
    if (soldSkus.has(sku)) continue;
    const qc = inv(sku);
    products.push({
      name:'(Sin venta) SKU '+sku, sku, units:0, units_prep:0, revenue:0, orders:0, unit_price:0,
      c34:0,c35:0,c36:0,c37:0,c38:0,na:0,
      inv_c34:invC(sku,'34'), inv_c35:invC(sku,'35'), inv_c36:invC(sku,'36'),
      inv_c37:invC(sku,'37'), inv_c38:invC(sku,'38'),
      q_cont:qc, disponible:qc
    });
  }
  // Quedarse solo con productos con relevancia preventa (venta o inventario en contenedor no llegado)
  const productsPreventa = products.filter(p => p.units>0 || p.q_cont>0);
  productsPreventa.sort((a,b)=>b.revenue-a.revenue);
  ...
  const monthly=[...mm.entries()].sort().map(([m,d])=>{
    ...
    return {month:(mN[m.substring(5,7)]||m.substring(5,7))+' '+m.substring(2,4),
      preventa_orders:po,prepared_orders:pp,preventa_revenue:pr};
  });
  ...
  return {
    ...
    container_stats:cs,
    container_inventory:{
      c34:ARRIVED.has('34')?0:sumInv(C34I), c35:ARRIVED.has('35')?0:sumInv(C35I),
      c36:ARRIVED.has('36')?0:sumInv(C36I), c37:ARRIVED.has('37')?0:sumInv(C37I),
      c38:ARRIVED.has('38')?0:sumInv(C38I)},
    preventa_containers:PREVENTA,
    arrived_containers:[...ARRIVED],
    monthly,lines:deduped,products:productsPreventa
  };
}
